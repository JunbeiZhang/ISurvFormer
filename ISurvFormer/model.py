...
class ColumnwiseImputer(nn.Module):
    """
    Column-wise LSTM imputer.

    For each feature dimension d, this module uses an LSTM that takes
    all other features (except d) as input to predict feature d.
    """
    ...
    def __init__(self, input_dim, hidden_dim=64, num_layers=1):
        ...
        self.output_layers = nn.ModuleList(
            [nn.Linear(hidden_dim, 1) for _ in range(input_dim)]
        )

    def forward(self, x_raw, mask_pad, mask_raw=None):
        """
        Args:
            x_raw:    [B, T, D] raw input with missing values.
            mask_pad: [B, T] boolean mask for valid time steps (True = valid, False = padding).
            mask_raw: [B, T, D] boolean mask for observed entries (True = observed).

        Returns:
            x_hat: [B, T, D] imputed sequence (full prediction for all entries).
        """
        B, T, D = x_raw.shape
        x_hat_list = []

        for d in range(D):
            # Indices of all other features except d
            other_idx = [i for i in range(D) if i != d]

            # Use all other features as input to predict feature d: [B, T, D-1]
            x_input = x_raw[:, :, other_idx].clone()

            # Mask out padding positions in all other features
            x_input[~mask_pad.unsqueeze(-1).expand(-1, -1, len(other_idx))] = 0

            # Optionally mask out missing entries in other columns to prevent information leakage
            if mask_raw is not None:
                # True = observed for other features
...
class TimeEmbedding(nn.Module):
    """
    Time embedding module using either learnable or sinusoidal (positional) encoding.
    """
    ...
    def forward(self, t):
        """
        Args:
            t: [B, T, 1] or [B, T] time input.

        Returns:
            pe: [B, T, d_model] time embeddings.
        """
...
def make_mlp_head(
    ...
    Args:
        ...
        activation: Activation name, passed to get_activation.
...
class DynamicSurvTransformer(nn.Module):
    """
    Dynamic survival model combining:
    - Column-wise LSTM imputation
    - MLP feature embedding
    - Time embedding
    - Transformer encoder
    - Risk head (for Cox loss)
    - Time-to-event (TTE) regression head
    """
    ...
    def __init__(
        ...
        num_layers=2,
        dropout=0.1,
        time_embed="positional",
        mlp_hidden_ratio=1.0,
        mlp_num_layers=2,
        activation="ReLU",
        risk_hidden_dim=64,
        risk_num_layers=2,
        risk_activation="ReLU",
    ):
        super().__init__()

        # Input MLP to project raw features to d_model
    ...
    def forward(self, x_raw, mask_raw, t, mask_pad):
        """
        Args:
            x_raw:    [B, T, D] raw input sequence with missing values.
            mask_raw: [B, T, D] boolean mask for observed entries (True = observed).
            t:        [B, T] or [B, T, 1] time indices.
            mask_pad: [B, T] boolean mask (True = valid, False = padding).

        Returns:
            h_seq:   [B, T, d_model] hidden sequence from Transformer.
            risk_seq: [B, T] risk scores for each time step.
            tte_seq:  [B, T] TTE regression outputs.
            x_tilde:  [B, T, D] final input used (raw on observed, imputed on missing).
        """
        # 1) Predict imputed values using column-wise LSTM
        #    (with optional masking of other columns' missing entries to prevent information leakage)
        x_filled = self.imputer(x_raw, mask_pad, mask_raw=mask_raw)  # [B, T, D]

        # 2) Only fill missing entries; keep original observed values
        #    mask_raw: [B, T, D] boolean; convert if needed
        m = mask_raw.bool()
        x_tilde = torch.where(m, x_raw, x_filled)

        # 3) Use x_tilde (mixed raw/imputed) for all subsequent modules
        x_embed = self.input_mlp(x_tilde)
        ...
        # Return x_tilde to enable imputation loss computation outside
        return h_seq, risk_seq, tte_seq, x_tilde


def cox_partial_log_likelihood(risk, durations, events):
    """
    Compute Cox partial log-likelihood (negative, to be used as loss).

    Args:
        risk:      [N] risk scores.
        durations: [N] event or censoring times.
...
def dynamic_survival_loss_with_imputation(
    ...
    """
    Composite loss for dynamic survival with imputation:

    Loss = Cox partial log-likelihood (on last step)
    ...
    Notes:
        - Imputation loss follows the "observed-supervised" design:
          supervise ONLY at positions that are originally observed and within
          the non-padded region; missing and padded positions are not supervised.
    """
    ...
    valid_time_mask = (
        torch.arange(L, device=device).view(1, L) < lengths.view(B, 1)
    )  # [B, L]
    ...
    non_pad_3d = (
        torch.arange(L, device=device).view(1, L, 1) < lengths.view(B, 1, 1)
    )  # [B, L, 1]
...
def generate_mask(lengths, max_len=None):
    """
    Generate a boolean mask for variable-length sequences.

    Args:
        lengths: [B] lengths of each sequence.
        max_len: Optional maximum length; if None, use max(lengths).

    Returns:
        mask: [B, max_len] boolean mask (True = valid, False = padding).
    """
    B = lengths.size(0)
    if max_len is None:
