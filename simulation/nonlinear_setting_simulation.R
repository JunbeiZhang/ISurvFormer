...
insp.rate <- 2           # Rate parameter for inspection times
...
# Generate longitudinal times and restrict to maximum time length
times <- replicate(
  N,
  {
    temp_times <- cumsum(c(0, rexp(n = K - 1, rate = insp.rate)))
    temp_times[temp_times <= max_time_length]  # Restrict time points to be at most max_time_length
  },
  simplify = FALSE
)

# Convert times to data frame
times_dat <- data.frame(id = rep(1:N, sapply(times, length)), time = unlist(times))

# Baseline covariate part
...
for (i in 1:N) {
  Root <- try(
    uniroot(
      invS,
      interval = c(1e-05, max_time_length),
      u = u[i],
      i = i,
      extendInt = "upX"
    )$root,
...
Time_dat <- data.frame(
  id = rep(1:N, sapply(times, length)),
  Time = rep(Time, sapply(times, length))
)
...
# Add observation at event time for subjects with an event
for (i in 1:N) {
  if (event[i] == 1) {
    event_time <- Time[i]
    ...
    # Simulate new longitudinal measurement at event time
    ...
    # Ensure consistent columns and bind to existing data
